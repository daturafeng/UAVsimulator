...
FMQTTURL FMQTTURL::FromString(const FString& InURL)
{
	...
	if(DelimeterIdx >= 0)
	{
		...
		static UEnum* SchemeEnum = FindObject<UEnum>(nullptr, TEXT("UAVMQTTCore.EMQTTScheme"));
...
{ 
	...
	// scheme ie. mqtt://
	Result.Append(UEnum::GetValueAsString(TEXT("UAVMQTTCore.EMQTTScheme"), Scheme).ToLower() + "://");
