...
系统 MUST 为每条 services 指令生成结果（result=0 成功，非 0 失败）与对应的事件（`takeoff_to_point_progress`、`flighttask_progress`），事件携带进度字段（如 status、progress、current_waypoint_index）供桥接层转发。
